...
from RL.preprocess_Data import load_gen_data, patch_text2data
...
def eval_reward_epoch(args, configs, eval_data, eval_examples, reinforce):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval reward"):
        ...
        with torch.no_grad():
            ...
            #     logger.info(text)
            patch_ids, patch_mask = patch_text2data(args, pred_nls, configs.reward[2])
            reward = reinforce.module.evaluate(buggy_ids, buggy_mask, patch_ids, patch_mask)
            smoothed_reward = reward_buffer.smooth(reward.cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_load_path", default=None, type=str,
                        help="Path to trained model: Should contain the .bin files")
    parser.add_argument("--reward_max_length", default=64, type=int,
                        help="The maximum total sequence length after tokenization. Sequences longer "
                             "than this will be truncated, sequences shorter will be padded.")
    ...
    for cur_epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            # logger.info(logits.shape, patch_out.shape)
            actions = patch_out[:, 1:]
